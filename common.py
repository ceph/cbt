...
def sync_files(remote_dir, local_dir):
    ...
    pdsh(nodes, 'sudo chown -R %s.%s %s' % (settings.cluster.get('user'),
                                            settings.cluster.get('user'),
                                            remote_dir))
...
def setup_valgrind(mode, name, tmp_dir):
    ...
    pdsh(settings.getnodes('clients', 'osds', 'mons', 'rgws', 'mds'),
         'mkdir -p -m0755 -- %s' % valdir).communicate()
    ...
    if mode == 'memcheck':
        return ('valgrind --tool=memcheck --soname-synonyms=somalloc=*tcmalloc*' +
                ' --log-file=%s ' % (logfile))
