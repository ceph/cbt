...
class checked_Popen:
	...
	self.continue_if_error = continue_if_error
        ...
        if self.loglevel: print('%s: %s' % (time.ctime(), ' '.join(args)))
    ...
    def communicate(self):
       ...
       if (self.myrtncode != self.OK) and (not self.turn_off_checking):
           if not self.continue_if_error:
               ...
               print('%s: error %d seen, continuing anyway...'%(time.ctime(), self.myrtncode))
       return (stdoutdata, stderrdata)

def pdsh(nodes, command, continue_if_error=False):
    args = ['pdsh', '-R', 'ssh', '-w', nodes, '%s'%command]
    # -S means pdsh fails if any host fails 
    if not continue_if_error: args.insert(1, '-S')
    return checked_Popen(args,continue_if_error=continue_if_error)
...
def pkill(nodelist, signalstr, process_name_pattern):
    pdsh(nodelist, 'sudo pkill %s -f %s'%(signalstr, process_name_pattern),continue_if_error=True).communicate()
