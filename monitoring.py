...
def start(directory):
    ...
    #    common.pdsh(osds, ('cd %s;sudo blktrace -o device%s -d' +
...
def stop(directory=None):
    ...
    if directory:
        sc = settings.cluster
        common.pdsh(nodes, 'cd %s/perf;sudo chown %s.%s perf.data' %
                    (directory, sc.get('user'), sc.get('user')))
...
def make_movies(directory):
    ...
    for device in range(sc.get('osds_per_node')):
        common.pdsh(settings.getnodes('osds'),
