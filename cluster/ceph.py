...
class OsdThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            common.pdsh(
                phost,
                "sudo %s auth get-or-create osd.%s mon 'allow rwx' osd 'allow *' -o %s"
                % (self.cl_obj.ceph_cmd, self.osdnum, key_fn),
            ).communicate()

            common.pdsh(
                phost,
                "sudo %s -c %s osd crush add osd.%d 1.0 host=%s rack=localrack root=default"
                % (self.cl_obj.ceph_cmd, ceph_conf, self.osdnum, self.host),
            ).communicate()
            cmd = "ulimit -n 16384 && ulimit -c unlimited && exec %s -c %s -i %d --mkfs --osd-uuid %s" % (
                ceph_osd_cmd,
                ceph_conf,
                self.osdnum,
                self.osduuid,
            ...
            if self.cl_obj.osd_valgrind:
                cmd = (
                    common.setup_valgrind(self.cl_obj.osd_valgrind, "osd.%d" % self.osdnum, self.cl_obj.tmp_dir)
                    + " "
                    + cmd
                )
            else:
            ...
            common.pdsh(
                phost,
                'sudo sh -c "ulimit -n 16384 && ulimit -c unlimited && exec %s > %s 2> %s < /dev/null &"'
    ...
    def postprocess(self):
        ...
        logger.debug(
            "thread %s completed creation of OSD %d elapsed time %f" % (self.name, self.osdnum, self.response_time)
        )
...
class Ceph(Cluster):
    ...
    def initialize(self):
        ...
        common.pdsh(
            settings.getnodes("head", "clients", "mons", "osds", "rgws", "mdss", "mgrs"),
            "mkdir -p -m0755 -- %s" % self.tmp_dir,
        ).communicate()
        common.pdsh(
            settings.getnodes("clients", "mons", "osds", "rgws", "mdss", "mgrs"), "mkdir -p -m0755 -- %s" % self.pid_dir
        ).communicate()
        common.pdsh(
            settings.getnodes("clients", "mons", "osds", "rgws", "mdss", "mgrs"), "mkdir -p -m0755 -- %s" % self.log_dir
        ).communicate()
        common.pdsh(
            settings.getnodes("clients", "mons", "osds", "rgws", "mdss", "mgrs"),
            "mkdir -p -m0755 -- %s" % self.monitoring_dir,
        ).communicate()
        common.pdsh(
            settings.getnodes("clients", "mons", "osds", "rgws", "mdss", "mgrs"),
            "mkdir -p -m0755 -- %s" % self.core_dir,
        ).communicate()
        ...
        common.pdsh(
            settings.getnodes("clients", "mons", "osds", "rgws", "mdss", "mgrs"),
            'echo "%s/core.%%e.%%p.%%h.%%t" | sudo tee /proc/sys/kernel/core_pattern' % self.tmp_dir,
        ).communicate()
        ...
        if self.idle_duration > 0:
            logger.info("Starting idle monitoring for %ds...", self.idle_duration)
            MonitoringFactory.start("%s/idle_monitoring" % self.monitoring_dir)
            elapsed = 0
            while elapsed < self.idle_duration:
                step = min(30, self.idle_duration - elapsed)
                time.sleep(step)
                elapsed += step
                remaining = self.idle_duration - elapsed
                if remaining > 0:
                    logger.info("Idle monitoring: %ds remaining...", remaining)
            MonitoringFactory.stop()
        ...
        for device in range(0, sc.get("osds_per_node")):
            ...
            if fs == "tmpfs":
                ...
                if self.newstore_block:
                    mkfs_cmd += (
                        " ; sudo ln -s /dev/disk/by-partlabel/osd-device-%s-block %s/osd-device-%s-data/block"
    ...
    def make_mons(self):
        ...
        common.pdsh(
            settings.getnodes("head", "clients", "osds", "mons", "rgws", "mgrs"), "rm -rf %s" % client_admin_dir
        ).communicate()
        common.pdsh(
            settings.getnodes("head", "clients", "osds", "mons", "rgws", "mgrs"), "mkdir -p %s" % client_admin_dir
        ).communicate()

        keyring_fn = os.path.join(client_admin_dir, "keyring")
        common.pdsh(
            settings.getnodes("head"),
            "%s --create-keyring --gen-key --name=mon. %s --cap mon 'allow *'" % (self.ceph_authtool_cmd, keyring_fn),
        ).communicate()
        common.pdsh(
            settings.getnodes("head"),
            "%s --gen-key --name=client.admin --cap mon 'allow *' --cap osd 'allow *' --cap mds 'allow *' --cap mgr 'allow *' %s"
            % (self.ceph_authtool_cmd, keyring_fn),
        ).communicate()
        common.rscp(settings.getnodes("head"), keyring_fn, "%s.tmp" % keyring_fn).communicate()
        common.pdcp(
            settings.getnodes("head", "clients", "osds", "mons", "rgws", "mgrs"), "", "%s.tmp" % keyring_fn, keyring_fn
        ).communicate()
        common.pdsh(
            settings.getnodes("head", "clients", "osds", "mons", "rgws", "mgrs"),
            "sudo mv %s %s.cbt.bak" % (self.client_keyring, self.client_keyring),
        ).communicate()
        common.pdsh(
            settings.getnodes("head", "clients", "osds", "mons", "rgws", "mgrs"),
            "sudo ln -s %s %s" % (keyring_fn, self.client_keyring),
        ).communicate()
        common.pdsh(
            settings.getnodes("head", "clients", "osds", "mons", "rgws", "mgrs"),
            "sudo mv %s %s.cbt.bak" % (self.client_secret, self.client_secret),
        ).communicate()
        common.pdsh(
            settings.getnodes("head", "clients", "osds", "mons", "rgws", "mgrs"),
            "sudo sh -c '%s --print-key %s > %s'" % (self.ceph_authtool_cmd, self.client_keyring, self.client_secret),
        ).communicate()
        ...
        for monhost, mons in monhosts.items():
            ...
            for mon, addr in mons.items():
                ...
                common.pdsh(
                    ...
                    % (self.ceph_mon_cmd, self.tmp_conf, mon, self.monmap_fn, keyring_fn),
                ).communicate()
        ...
        for monhost, mons in monhosts.items():
            ...
            for mon, addr in mons.items():
                ...
                cmd = (
                    'sudo sh -c "ulimit -n 16384 && ulimit -c unlimited && exec %s -c %s -i %s --keyring=%s --pid-file=%s"'
                    % (self.ceph_mon_cmd, self.tmp_conf, mon, keyring_fn, pidfile)
                )
    ...
    def make_osds(self):
        ...
        logger.info(
            "OSD creation rate: < %d OSDs/sec , join timeout %d, parallel creates < %s"
            % (self.ceph_osd_online_rate, self.ceph_osd_online_tmo, str(self.ceph_osd_parallel_creates))
        )
        ...
        for host in osdhosts:
            for devnumstr in range(0, settings.cluster.get("osds_per_node")):
                ...
                common.pdsh(
                    pdshhost, "sudo %s -c %s osd create %s" % (self.ceph_cmd, self.tmp_conf, osduuid)
                ).communicate()
                # bring the OSD online in background while continuing to create OSDs in foreground
                thrd = OsdThread(
                    self, devnumstr, osdnum, clusterid, host, osduuid, osddir, self.tmp_dir, crimson_cpuset
                )
                logger.debug("starting creation of OSD %d " % osdnum)
                thrd.start()
        ...
        for mgrhost, manager in mgrhosts.items():
            for mgrname, mgrsettings in manager.items():
                ...
                common.pdsh(
                    ...
                    % (self.ceph_cmd, mgrname, data_dir),
                ).communicate()
                common.pdsh(
                    pdshhost, 'sudo sh -c "ulimit -n 16384 && ulimit -c unlimited && exec %s"' % cmd
                ).communicate()
    ...
    def start_mds(self):
        ...
        for mdshost, mds in mdshosts.items():
            for mdsname, mdssettings in mds.items():
                ...
                common.pdsh(
                    pdshhost,
                    "sudo %s auth get-or-create mds.%s mon 'allow profile mds' osd 'allow rw tag cephfs *=*' mds 'allow' mgr 'allow profile mds' -o %s/keyring"
                    % (self.ceph_cmd, mdsname, data_dir),
                ).communicate()
                common.pdsh(
                    pdshhost, 'sudo sh -c "ulimit -n 16384 && ulimit -c unlimited && exec %s"' % cmd
                ).communicate()
            ...
            for rgwname, rgwsettings in gateways.items():
                ...
                if num_osds == 1:
                    ...
                    for pool in rgw_default_pools:
                        common.pdsh(
                            settings.getnodes("head"),
                            "sudo %s -c %s osd pool set %s min_size %d"
                            % (self.ceph_cmd, self.tmp_conf, pool, pool_min_repl_size),
    ...
    def check_health(self, check_list=None, logfile=None, recstatsfile=None):
        ...
        if recstatsfile:
            ...
            stdout, stderr = common.pdsh(
                settings.getnodes("head"), "echo %s >> %s" % (header, recstatsfile)
            ).communicate()
        ...
        while True:
            stdout, stderr = common.pdsh(
                settings.getnodes("head"), "%s -c %s health %s" % (self.ceph_cmd, self.tmp_conf, logline)
            ).communicate()
            ...
            poll_count += 1
            if poll_count % 30 == 0:
                logger.info("Still waiting for cluster health... (%d polls)", poll_count)
    ...
    def log_recovery_stats(self, recstatsfile=None):
        ...
        separator = ","
        stdout, stderr = common.pdsh(
            settings.getnodes("head"), "%s -c %s -s %s" % (self.ceph_cmd, self.tmp_conf, fmtjson)
        ).communicate()
        ...
        if len(degstats):
            ...
            stdout, stderr = common.pdsh(
                settings.getnodes("head"), "echo %s >> %s" % (message, recstatsfile)
            ).communicate()
    ...
    def check_backfill(self, check_list=None, logfile=None, recstatsfile=None):
        ...
        while True:
            stdout, stderr = common.pdsh(
                settings.getnodes("head"), "%s -c %s -s %s" % (self.ceph_cmd, self.tmp_conf, logline)
            ).communicate()
            if check_list and not any(x in stdout for x in check_list):
                break
            else:
    ...
    def check_scrub(self):
        ...
        while True:
            stdout, stderr = common.pdsh(
                settings.getnodes("head"),
                '%s -c %s pg dump | cut -f 16 | grep "0.000000" | wc -l' % (self.ceph_cmd, self.tmp_conf),
            ).communicate()
            if " 0\n" in stdout:
                break
            else:
    ...
    def dump_config(self, run_dir):
        common.pdsh(
            ...
            "sudo %s -c %s daemon osd.0 config show > %s/ceph_settings.out" % (self.ceph_cmd, self.tmp_conf, run_dir),
        ).communicate()

    def dump_historic_ops(self, run_dir):
        common.pdsh(
            settings.getnodes("osds"),
            "find /var/run/ceph/ceph-osd*.asok -maxdepth 1 -exec sudo %s --admin-daemon {} dump_historic_ops \; > %s/historic_ops.out"
            % (self.ceph_cmd, run_dir),
        ).communicate()
    ...
    def set_osd_param(self, param, value):
        ...
        logger.info("OSD param '%s' set to '%s'.", param, value)
    ...
    def create_recovery_test(self, run_dir, callback, test_type="blocking"):
        ...
        if test_type == "blocking":
            ...
            self.rt = RecoveryTestThreadBackground(
                rt_config, self, callback, self.stoprequest, self.haltrequest, self.startiorequest
            )
        self.rt.start()
    ...
    def maybe_populate_recovery_pool(self):
        if self.prefill_recov_objects > 0 or self.prefill_recov_time > 0:
            logger.info(
                "prefilling %s %sbyte objects into recovery pool %s"
                % (self.prefill_recov_objects, self.prefill_recov_object_size, self.recov_pool_name)
            )
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -p %s bench %s write -b %s --max-objects %s --no-cleanup"
                % (
                    self.rados_cmd,
                    self.recov_pool_name,
                    self.prefill_recov_time,
                    self.prefill_recov_object_size,
                    self.prefill_recov_objects,
                ),
            ).communicate()
            self.check_health()
    ...
    def check_pg_autoscaler(self, timeout=-1, logfile=None):
        ...
        logger.info("Waiting 20s before checking PG autoscaler...")
        time.sleep(20)
        start_time = time.time()
        logger.info("Checking PG autoscaler...")
        while True:
            stdout, stderr = common.pdsh(
                settings.getnodes("head"), "%s;%s -c %s progress %s" % (logdate, self.ceph_cmd, self.tmp_conf, logline)
            ).communicate()
            output = (stdout, stderr)
            for i in range(len(output)):
                if any(pg_state in output[i] for pg_state in pg_autoscaler_states):
                    logger.info("PG autoscaler complete.")
                    return ret
            elapsed = time.time() - start_time
            if timeout > 0:
                if elapsed > timeout:
                    logger.warning("check_pg_autoscaler() state timeout exceeded...")
                    ret = ret + 1
                    return ret
            logger.info("PG autoscaler still running... (%.0fs elapsed)", elapsed)
            logger.debug("%s", stdout)
            time.sleep(10)
        return ret
    ...
    def get_ruleset(self, name):
        name = str(name)
        logger.debug("%s", self.ruleset_map)
    ...
    def make_profiles(self):
        ...
        for name, profile in list(crush_profiles.items()):
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd crush add-bucket %s-root root" % (self.ceph_cmd, self.tmp_conf, name),
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd crush add-bucket %s-rack rack" % (self.ceph_cmd, self.tmp_conf, name),
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd crush move %s-rack root=%s-root" % (self.ceph_cmd, self.tmp_conf, name, name),
            ).communicate()
            # FIXME: We need to build a dict mapping OSDs to hosts and create a proper hierarchy!
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd crush add-bucket %s-host host" % (self.ceph_cmd, self.tmp_conf, name),
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd crush move %s-host rack=%s-rack" % (self.ceph_cmd, self.tmp_conf, name, name),
            ).communicate()
            ...
            for i in osds:
                common.pdsh(
                    settings.getnodes("head"),
                    "%s -c %s osd crush set %s 1.0 host=%s-host" % (self.ceph_cmd, self.tmp_conf, i, name),
                ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd crush rule create-simple %s %s-root osd" % (self.ceph_cmd, self.tmp_conf, name, name),
            ).communicate()
        ...
        for name, profile in list(erasure_profiles.items()):
            ...
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd erasure-code-profile set %s crush-failure-domain=osd k=%s m=%s"
                % (self.ceph_cmd, self.tmp_conf, name, k, m),
            ).communicate()
            self.set_ruleset(name)
    ...
    def mkpool(self, name, profile_name, application, base_name=None):
        ...
        if replication and replication == "erasure":
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool create %s %d %d erasure %s %s"
                % (self.ceph_cmd, self.tmp_conf, name, pg_size, pgp_size, erasure_profile, yes_flag),
                continue_if_error=False,
            ).communicate()
            if ec_overwrites is True:
                common.pdsh(
                    settings.getnodes("head"),
                    "sudo %s -c %s osd pool set %s allow_ec_overwrites true" % (self.ceph_cmd, self.tmp_conf, name),
                    continue_if_error=False,
                ).communicate()
        else:
            common.pdsh(
                ...
                % (self.ceph_cmd, self.tmp_conf, name, pg_size, pgp_size, yes_flag),
                continue_if_error=False,
            ).communicate()
        if self.version_compat not in [
            "argonaut",
            ...
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool application enable %s %s" % (self.ceph_cmd, self.tmp_conf, name, application),
                continue_if_error=False,
            ).communicate()
        ...
        if replication and replication.isdigit():
            ...
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s size %s %s"
                % (self.ceph_cmd, self.tmp_conf, name, replication, yes_flag),
                continue_if_error=False,
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s min_size %d %s"
                % (self.ceph_cmd, self.tmp_conf, name, pool_min_repl_size, yes_flag),
                continue_if_error=False,
            ).communicate()
        ...
        if crush_profile:
            ...
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s crush_ruleset %s" % (self.ceph_cmd, self.tmp_conf, name, crush_profile),
                continue_if_error=False,
            ).communicate()
        ...
        if prefill_objects > 0 or prefill_time > 0:
            ...
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -p %s bench %s write -b %s --max-objects %s --no-cleanup"
                % (self.rados_cmd, name, prefill_time, prefill_object_size, prefill_objects),
            ).communicate()
            self.check_health()
        ...
        if base_name and cache_mode:
            ...
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd tier add %s %s" % (self.ceph_cmd, self.tmp_conf, base_name, name),
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd tier cache-mode %s %s" % (self.ceph_cmd, self.tmp_conf, name, cache_mode),
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd tier set-overlay %s %s" % (self.ceph_cmd, self.tmp_conf, base_name, name),
            ).communicate()

        if hit_set_type:
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s hit_set_type %s" % (self.ceph_cmd, self.tmp_conf, name, hit_set_type),
            ).communicate()
        if hit_set_count:
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s hit_set_count %s" % (self.ceph_cmd, self.tmp_conf, name, hit_set_count),
            ).communicate()
        if hit_set_period:
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s hit_set_period %s"
                % (self.ceph_cmd, self.tmp_conf, name, hit_set_period),
            ).communicate()
        if target_max_objects:
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s target_max_objects %s"
                % (self.ceph_cmd, self.tmp_conf, name, target_max_objects),
            ).communicate()
        if target_max_bytes:
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s target_max_bytes %s"
                % (self.ceph_cmd, self.tmp_conf, name, target_max_bytes),
            ).communicate()
        if min_read_recency_for_promote:
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s min_read_recency_for_promote %s"
                % (self.ceph_cmd, self.tmp_conf, name, min_read_recency_for_promote),
            ).communicate()
        if min_write_recency_for_promote:
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd pool set %s min_write_recency_for_promote %s"
                % (self.ceph_cmd, self.tmp_conf, name, min_write_recency_for_promote),
            ).communicate()
    ...
    def rmpool(self, name, profile_name):
        ...
        if cache_profile:
            ...
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd tier cache-mode %s forward" % (self.ceph_cmd, self.tmp_conf, cache_name),
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s -p %s cache-flush-evict-all" % (self.rados_cmd, self.tmp_conf, cache_name),
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd tier remove-overlay %s" % (self.ceph_cmd, self.tmp_conf, name),
            ).communicate()
            common.pdsh(
                settings.getnodes("head"),
                "sudo %s -c %s osd tier remove %s %s" % (self.ceph_cmd, self.tmp_conf, name, cache_name),
            ).communicate()
        ...
        common.pdsh(
            settings.getnodes("head"),
            "sudo %s -c %s osd pool delete %s %s --yes-i-really-really-mean-it"
            % (self.ceph_cmd, self.tmp_conf, name, name),
            continue_if_error=False,
        ).communicate()
    ...
    def mkimage(self, name, size, pool, data_pool, order):
        ...
        try:
            common.pdsh(
                ...
                % (self.rbd_cmd, self.tmp_conf, name, size, pool, dp_option, order),
                continue_if_error=False,
            ).communicate()
        except Exception as e:
            logger.error(str(e))
    ...
    def unmount_all(self):
        ...
        common.pdsh(
            settings.getnodes("clients"), 'sudo umount $(grep %s /proc/mounts | cut -f2 -d" " | sort -r)' % self.mnt_dir
        ).communicate()
        ...
        common.pdsh(
            settings.getnodes("clients"), "sudo find /dev/rbd* -maxdepth 0 -type b -exec umount '{}' \\;"
        ).communicate()
        common.pdsh(
            settings.getnodes("clients"), "sudo find /dev/nbd* -maxdepth 0 -type b -exec umount '{}' \\;"
        ).communicate()

        # Unmap rbd, nbd, and clear the targetcli config
        common.pdsh(
            settings.getnodes("clients"),
            "sudo find /dev/rbd* -maxdepth 0 -type b -exec %s unmap '{}' \\;" % self.rbd_cmd,
        ).communicate()
        common.pdsh(
            settings.getnodes("clients"),
            "sudo find /dev/nbd* -maxdepth 0 -type b -exec %s unmap '{}' \\;" % self.rbd_nbd_cmd,
        ).communicate()
        common.pdsh(
            settings.getnodes("clients"), "sudo targetcli clearconfig confirm=True", continue_if_error=False
        ).communicate()
    ...
    def add_s3_user(self, user, access_key, secret_key):
        if self.urls:
            ...
            common.pdsh(
                node, "%s -c %s user create --uid=%s --display-name=%s" % (cmd, self.tmp_conf, user, user)
            ).communicate()
            common.pdsh(
                node,
                "%s -c %s key create --uid=%s --key-type=s3 --access_key=%s --secret_key=%s"
                % (cmd, self.tmp_conf, user, access_key, secret_key),
            ).communicate()
            common.pdsh(
                node, "%s -c %s user modify --uid=%s --max-buckets=0" % (cmd, self.tmp_conf, user)
            ).communicate()
    ...
    def add_swift_user(self, user, subuser, key):
        if self.auth_urls:
            ...
            common.pdsh(
                node,
                "%s -c %s subuser create --uid=%s --subuser=%s --access=full" % (cmd, self.tmp_conf, user, subuser),
            ).communicate()
            common.pdsh(
                node,
                "%s -c %s key create --subuser=%s --key-type=swift --secret=%s" % (cmd, self.tmp_conf, subuser, key),
            ).communicate()
            common.pdsh(
                node, "%s -c %s user modify --uid=%s --max-buckets=0" % (cmd, self.tmp_conf, user)
            ).communicate()
...
class RecoveryTestThreadBlocking(threading.Thread):
    ...
    def pre(self):
        ...
        common.pdsh(
            settings.getnodes("head"), self.logcmd("Starting Recovery Test Thread, waiting %s seconds." % pre_time)
        ).communicate()
        time.sleep(pre_time)
        lcmd = self.logcmd("Setting the ceph osd noup flag")
        common.pdsh(
            settings.getnodes("head"), "%s -c %s osd set noup;%s" % (self.ceph_cmd, self.cluster.tmp_conf, lcmd)
        ).communicate()
        self.state = "markdown"
    ...
    def markdown(self):
        for osdnum in self.config.get("osds"):
            ...
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd down %s;%s" % (self.ceph_cmd, self.cluster.tmp_conf, osdnum, lcmd),
            ).communicate()
            lcmd = self.logcmd("Marking OSD %s out." % osdnum)
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd out %s;%s" % (self.ceph_cmd, self.cluster.tmp_conf, osdnum, lcmd),
            ).communicate()
    ...
    def osdout(self):
        ...
        common.pdsh(
            settings.getnodes("head"), "%s -c %s osd unset noup;%s" % (self.ceph_cmd, self.cluster.tmp_conf, lcmd)
        ).communicate()
        for osdnum in self.config.get("osds"):
            lcmd = self.logcmd("Marking OSD %s up." % osdnum)
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd up %s;%s" % (self.ceph_cmd, self.cluster.tmp_conf, osdnum, lcmd),
            ).communicate()
            lcmd = self.logcmd("Marking OSD %s in." % osdnum)
            common.pdsh(
                settings.getnodes("head"),
                "%s -c %s osd in %s;%s" % (self.ceph_cmd, self.cluster.tmp_conf, osdnum, lcmd),
            ).communicate()
        self.lasttime = time.time()
        self.state = "osdin"
    ...
    def osdin(self):
        ...
        if ret == 0:
            ...
            common.pdsh(settings.getnodes("head"), self.logcmd("Cluster appears to have healed.")).communicate()
            common.pdsh(
                settings.getnodes("head"), self.logcmd("Time: %s" % str(time.time() - self.lasttime))
            ).communicate()
        self.state = "post"
    ...
    def post(self):
        if self.stoprequest.isSet():
            common.pdsh(
                settings.getnodes("head"), self.logcmd("Cluster is healthy, but stoprequest is set, finishing now.")
            ).communicate()
        ...
        if self.config.get("repeat", False):
            ...
            common.pdsh(
                settings.getnodes("head"),
                self.logcmd('Cluster is healthy, but repeat is set.  Moving to "markdown" state.'),
            ).communicate()
            self.state = "markdown"
            return
        ...
        common.pdsh(
            settings.getnodes("head"), self.logcmd("Cluster is healthy, completion in %s seconds." % post_time)
        ).communicate()
    ...
    def join(self, timeout=None):
        common.pdsh(
            settings.getnodes("head"), self.logcmd("Received notification that parent is finished and waiting.")
        ).communicate()
...
class RecoveryTestThreadBackground(threading.Thread):
    ...
    def osdout(self):
        ...
        common.pdsh(
            settings.getnodes("head"),
            self.logcmd("osdout state - Sleeping for 10 secs after populating recovery pool."),
        ).communicate()
        time.sleep(10)
        lcmd = self.logcmd("Unsetting the ceph osd noup flag")
        common.pdsh(
    ...
    def osdin(self):
        ...
        ret = self.cluster.check_backfill(
            self.health_checklist, "%s/recovery.log" % self.config.get("run_dir"), recstatslog
        )
        common.pdsh(settings.getnodes("head"), self.logcmd("ret: %s" % ret)).communicate()
