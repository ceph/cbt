...
class RbdFio(Benchmark):
    ...
    def __init__(self, archive_dir, cluster, config):
        ...
        self.run_dir = "%srbdfio/client_ra-%08d/op_size-%08d/concurrent_procs-%03d/iodepth-%03d/%s" % (
            self.run_dir,
            int(self.client_ra),
            int(self.op_size),
            int(self.concurrent_procs),
            int(self.iodepth),
            self.mode,
        )
        self.out_dir = "%s/rbdfio/osd_ra-%08d/client_ra-%08d/op_size-%08d/concurrent_procs-%03d/iodepth-%03d/%s" % (
            self.archive_dir,
            int(self.osd_ra),
            int(self.client_ra),
            int(self.op_size),
    ...
    def set_client_param(self, param, value):
        common.pdsh(
            ...
            'find /sys/block/rbd* -exec sudo sh -c "echo %s > {}/queue/%s" \;' % (value, param),
        ).communicate()
    ...
    def mkimages(self):
        ...
        common.pdsh(
            settings.getnodes("clients"),
            "/usr/bin/rbd create cbt-kernelrbdfio-`hostname -s` --size %s --pool %s" % (self.vol_size, self.poolname),
        ).communicate()
        common.pdsh(
            settings.getnodes("clients"),
            "sudo rbd map cbt-kernelrbdfio-`hostname -s` --pool %s --id admin" % self.poolname,
        ).communicate()
        common.pdsh(
            settings.getnodes("clients"), "sudo mkfs.xfs /dev/rbd/cbt-kernelrbdfio/cbt-kernelrbdfio-`hostname -s`"
        ).communicate()
        common.pdsh(
            settings.getnodes("clients"),
            "sudo mkdir -p -m0755 -- %s/cbt-kernelrbdfio-`hostname -s`" % self.cluster.mnt_dir,
        ).communicate()
        common.pdsh(
            settings.getnodes("clients"),
            "sudo mount -t xfs -o noatime,inode64 /dev/rbd/cbt-kernelrbdfio/cbt-kernelrbdfio-`hostname -s` %s/cbt-kernelrbdfio-`hostname -s`"
            % self.cluster.mnt_dir,
        ).communicate()
        MonitoringFactory.stop()
