...
class RawFio(Benchmark):
    ...
    def __init__(self, archive_dir, cluster, config):
        ...
        self.run_dir = "%sraw_ra-%08d/op_size-%08d/concurrent_procs-%03d/iodepth-%03d/%s" % (
            self.run_dir,
            int(self.osd_ra),
            int(self.op_size),
        ...
        self.out_dir = "%s/raw_ra-%08d/op_size-%08d/concurrent_procs-%03d/iodepth-%03d/%s" % (
            self.archive_dir,
            int(self.osd_ra),
            int(self.op_size),
            int(self.total_procs),
    ...
    def initialize(self):
        ...
        common.pdsh(
            settings.getnodes("clients"), "sudo rm -rf %s" % self.run_dir, continue_if_error=False
        ).communicate()
