...
class Radosbench(Benchmark):
    ...
    def __init__(self, archive_dir, cluster, config):
        ...
        self.run_dir = os.path.join(
            self.run_dir, "op_size-{:0>8}".format(self.op_size), "concurrent_ops-{:0>8}".format(self.concurrent_ops)
        )
    ...
    def estimate_duration(self) -> int:
        """Estimate total run-phase seconds: prefill + write + read phases."""
        total = 0
        if self.prefill_time or self.prefill_objects:
            total += int(self.prefill_time or self.time)
        if not self.read_only:
            total += int(self.write_time)
        if not self.write_only:
            total += int(self.read_time)
        return total
    ...
    def get_rados_version(self):
        ...
        m = (
            re.findall(r"version (\d+)(?:.\d+)* \([0-9a-f]+\)", stdout)
            or re.findall(r"version v(\d+)(?:.\d+)* \([0-9a-f]+\)", stdout)
            or (255, 0)
        )
        return int(m[0])
    ...
    def run(self):
        ...
        if self.read_only and self.write_only:
            logger.error('Both "read_only" and "write_only" are specified, ' "but they are mutually exclusive.")
            return
        elif self.read_only and not do_prefill:
            logger.error(
                'Please prefill the testbench with "prefill_time" and/or '
        ...
        if not self.read_only:
            self._run(
                mode="write", run_dir="write", out_dir="write", max_objects=self.max_objects, runtime=self.write_time
            )
        # Run read test unless write_only
        if not self.write_only:
            self._run(
                mode=self.readmode,
                run_dir=self.readmode,
                out_dir=self.readmode,
                max_objects=None,
            ...
            for i in range(self.concurrent_procs):
                ...
                if self.pool_per_proc:
                    ...
                    run_name = run_name_fmt.format(
                        object_set_id=self.object_set_id, fqdn_cmd=common.get_fqdn_cmd(), i=i
                    )
                rados_bench_cmd_fmt = (
